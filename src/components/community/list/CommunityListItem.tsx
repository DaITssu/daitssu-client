  ...
  return (
    <>
      <Link href={`/community/${item.id}`}>
        <styles.ListBox>
          <styles.Status>{item.topic}</styles.Status>
          <styles.Title>{item.title}</styles.Title>
          <styles.Contents>{item.content}</styles.Contents>
          <styles.Bubble>
            <styles.BubbleGroup
              src={bubble}
              width={18}
              ...
              alt={'chat_icon'}
            />
            <styles.Review>{item.comments}</styles.Review>
          </styles.Bubble>

          <styles.Minutes>{changeDate(item.updatedAt)}</styles.Minutes>
        </styles.ListBox>
      </Link>
    </>
  );
