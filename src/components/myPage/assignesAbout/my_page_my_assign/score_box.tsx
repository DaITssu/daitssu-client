...
export default function AssignDetailScoreBox(f:FormData) {
  const [formData, setFormData] = useState<FormData>({
    title: f.title,
    dateString1: f.dateString1,
    dataString2: f.dataString2,
    width:f.width,
    isEnded:f.isEnded
  });
  return (
    ...
    <styles.MyAssignTitleBox width={formData.width} height={80}>
      ...
      <div style={{height:"10px"}}></div>
        ...
        <div style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
          <span>
            <styles.MyAssignTitleBoxText color={COLORS.SSU.primary}>
              {formData.dateString1}
            </styles.MyAssignTitleBoxText>
          </span>
          <span style={{marginLeft:"3px"}}>
          <styles.MyAssignTitleBoxText>
            {formData.dataString2}
          </styles.MyAssignTitleBoxText>
          </span>
        </div>
